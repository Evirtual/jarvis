...
| `src/client/board-store.ts`, `core-canvas.ts`, `stage-pointer.ts`, `stage-phone.ts` | The stage's parts: the workspace loaded and saved (and a conversation kept at the core by an earlier version brought in as a thread); JARVIS drawn and where he is; carrying, sizing and dropping on the desktop (what a drop means); the list a phone shows instead, its order and the grip |
...
| `src/client/ask.ts` | The command line, the queue, what JARVIS is told, and the exchange in named pieces — the thread it goes into (the one in front, or a new one), stream, finish, fail, carry out |
| `src/client/commands.ts` | The console's own commands — the few that must work with nothing connected — and the directives at the end of a reply |
...
| `src/client/local.ts` | Questions answered from live readings, never from a model — said under the core, not kept |
...
| `src/client/threads.ts`, `threads-panel.ts`, `board-links.ts` | What the stage reports back and the count; the Threads list; the links between threads on the board |
...
| `src/client/say.ts`, `address.ts` | The line under the core (a notice and the console's own answer, one box, neither kept), lines in a window, his status word; sir or ma'am |
