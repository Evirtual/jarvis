...
import { averageHues, DEFAULT_TITLE, GENERAL_ID, Workspace, migrate } from "../src/client/workspace.ts";
...
test("a thread opened without a name is called New thread until it is given one", () => {
  const ws = new Workspace();
  const t = ws.createThread({});
  assert.equal(t.title, DEFAULT_TITLE);
  ws.rename(t.id, "Lisbon in October");
  assert.equal(t.title, "Lisbon in October");
  assert.equal(ws.createThread({ title: "Named" }).title, "Named");
});
