...
/* ---------------- the voice controls in Configuration ---------------- */
...
/** Listening: how long a pause ends it, or whether only a tap does. Both remembered; the pause slider is put away while stopping is by hand. */
...
/** Esc, the tap on JARVIS, the keyboard, what the voice reports, and every control in Configuration → Voice. */
export function wireVoiceUi(): void {
  ...
  voice.onState = (): void => {
    ...
    const sliders = pitchSl.closest<HTMLElement>(".ctl-row");
  ...
  // Painted once now: a browser with no voices of its own and nothing connected would otherwise never say so.
  voice.onState();
}
