 ...
 * copy. Connect takes this window to OpenRouter to sign in — or to make a free
 * account — and OpenRouter sends it back here with a one-time code, which
 ...
 * device. The same window, always: in a browser tab and in an installed app
 * alike, so the key lands in the storage of the app that asked for it. The
 * board is saved, so the one reload on the way back loses nothing.
 */
...
/** Off to OpenRouter. The secret half of the exchange stays on this device until it comes back. */
export async function startOpenRouter(): Promise<void> {
  ...
  try { localStorage.setItem(PENDING, JSON.stringify({ verifier, method, at: Date.now() })); } catch { /* private mode */ }
...
export async function finishOpenRouter(): Promise<"none" | "connected" | string> {
  ...
  const { verifier, method, at } = JSON.parse(pending) as { verifier: string; method: "S256" | "plain"; at?: number };
